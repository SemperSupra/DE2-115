...
| USB host mode Type-A | Blocked | Beagle USB 12 is inline between KVM2USB and the DE2-115 HOST port; passive captures show no packets without a fresh reconnect event. 2026-05-17 Terasic host demo audit programmed the Terasic SOF, but the required Nios ELF download did not complete in Docker because the container lacks USB/JTAG passthrough and native Linux Nios helper tools. | Fix HPI readback, then LCP load, `COMM_ACK`, `SIE1_INIT`, reset, connect message, and HID boot protocol. For the Terasic reference demo, first provide a usable Nios ELF download path: native Linux Intel/Nios tools plus USB-Blaster passthrough in container, or host-side Windows Nios tools. |
