...
| USB host mode Type-A | Blocked | DE2 HOST path with KVM2USB inline shows repeated connect/disconnect/reset but no packets on both the project image and Terasic host mouse demo. KVM2USB validates on the PC hub path as `VID_2B77&PID_3661`, and Beagle sees real SETUP/descriptor/ACK/IN-NAK traffic there, though with repeated resets. | Next isolate the DE2 host path with a simple low/full-speed USB mouse or keyboard through the Beagle. Then fix CY reset/clock/power, HPI readback, LCP load, `COMM_ACK`, `SIE1_INIT`, reset, connect message, and HID boot protocol. |
| USB device mode Type-B | Blocked | Terasic `DE2_115_USB_DEVICE_LED.sof` does not enumerate reliably. One hub showed `Unknown USB Device (Device Descriptor Request Failed)` / `VID_0000&PID_0002`; clean Beagle inline capture shows `TGT_CONNECT/UNRST` followed by continuous `BAD_SYNC` and no valid packets, even after reprogramming and with KVM2USB unplugged from HOST. | First resolve physical/PHY path: verify Beagle orientation, use a short known-good Type-B cable, try direct PC port vs hub, and inspect CY power/reset/connector. Then return to LiteX device-mode firmware. |
