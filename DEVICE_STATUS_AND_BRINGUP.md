...
| USB CY7C67200 HPI read path | Blocked | CY registers and memory read as `0x0000`; source/probe shows valid read strobes; 2026-05-17 pad snapshot captured canonical read with `addr=0`, `CS_N=0`, `RD_N=0`, `WR_N=1`, `hpi_data=0x0000`. Board A reproduced the same failure after board B was swapped out, and reset/timing sweeps across `spec`, `fast`, and `slow` did not change canonical readback. | Compare with the Terasic USB demo under documented VBUS/jumper conditions and audit HPI reset/strap/protocol assumptions before any LCP/SIE work. |
| USB host mode Type-A | Blocked | Beagle USB 12 is inline between KVM2USB and the DE2-115 HOST port. 2026-05-17 Terasic reference demo now loads without WSL using the host-native SREC/`nios2-gdb-server.exe` path and starts the Nios CPU. AgentUSB2KVM direct HID injection works from the PC side, but Beagle captures `artifacts/usb-hpi-runs/20260517-173028/beagle12/capture_20260517-173031.txt` and `artifacts/usb-hpi-runs/20260517-180923/beagle12/capture_20260517-180935.txt` still show only connect/reset state changes and no SOF/SETUP packets. | Fix HPI readback before LiteX LCP work. For Terasic reference isolation, focus next on physical USB host-power, VBUS/jumper observations, and why the downstream target repeatedly resets without host token traffic. |
...
- Every device needs a firmware UART result and, where possible, an Etherbone
  host-driven test.
- Add source-probe or on-FPGA pad snapshots at the first uncertain hardware
  boundary. Use SignalTap only when the SOF exposes a working SignalTap node.
