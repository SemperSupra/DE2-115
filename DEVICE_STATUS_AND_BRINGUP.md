...
| USB host mode Type-A | Blocked | Beagle USB 12 is inline between KVM2USB and the DE2-115 HOST port; active captures with both the project image and Terasic host mouse demo show repeated connect/disconnect/reset but no packets. | Next isolate the downstream device/path with a simple low/full-speed USB mouse or keyboard through the Beagle. Then fix HPI readback, LCP load, `COMM_ACK`, `SIE1_INIT`, reset, connect message, and HID boot protocol. |
